...
THE_END:
  ...
  if (!success) {
    child->loadSuccess = 0;
    //printf("start_process !success, before sema_up\n");
    sema_up(&cur->parent->sema_load);
    //printf("start_process !success, after sema_up\n");
    palloc_free_page (file_name);
    thread_exit();
  } else {
    child->loadSuccess = 1;
    //printf("start_process success  %s  %d child %d\n",cur->name,  cur->tid, child->pid);
    cur->exefile = filesys_open(file_name);
    //printf("\n%s\n", file_name);
    //if (cur->exefile != NULL) 
    file_deny_write(cur->exefile);
    sema_up(&cur->parent->sema_load);
    palloc_free_page (file_name);
  }
...
//while(1);
  ...
  for (e = list_begin (&cur->children); e != list_end (&cur->children); e = list_next (e)) {
	...
	if (child->pid == child_tid) {
		...
		if (child->hasExit) {
			return_val = child->isKilled ? -1 : child->exit_status;
			child->hasWaited = 1;
			lock_release(&cur->l);
			return return_val;
		}
		//child->isWaiting = 1;
		//while (child->isWaiting) {
		while ((!child->hasExit) || (child->loadSuccess == 0)) {
			//printf("\nBEFORE WAIT\n");
			cond_wait(&cur->c, &cur->l);
			//printf("\nAFTER WAIT\n");
		}
		return_val = child->isKilled ? -1 : child->exit_status;
		child->hasWaited = 1;
...
process_exit (void)
{
  ...
  if (cur->parent != NULL) {
	...
	struct list children = cur->parent->children;
		...
		struct child *child = list_entry (e, struct child, childelem);
		//if (cur->tid == child->pid) child->hasExit = 1;
		if (cur->tid == child->pid) {// && child->isWaiting) {
			cond_signal(&cur->parent->c, &cur->parent->l);
			child->hasExit = 1;
			//child->isWaiting = 0;
			//printf("cond signal \n");
			break;
  ...
  while (!list_empty(&cur->children)) {
        struct child *child = list_entry(list_begin(&cur->children), struct child, childelem);
        child->t->parent = NULL;
